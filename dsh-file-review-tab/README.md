...
> **KCoder 收编 fork（@kcoder/file-review）**：本目录是 [Lzh3070/dsh-file-review-tab](https://github.com/Lzh3070/dsh-file-review-tab) 的源码镜像 + KCoder 维护线。上游作者维护缓慢，自 0.4.1 起由 KCoder 接管：包名改 `@kcoder/file-review`，并修复 dsh 0.1.2-alpha.1+ 基线的会话快照契约迁移（controller Session 快照只剩队列态，transcript 快照改从 `uiConversation.binding(id).target('chat')` 的 `legacy` 切片取，见 `src/client/conversation-store.ts`）。
>
> **维护流程**：改 `src/` → `pnpm install && pnpm build` → 将 `lib/`（去 `.map`/`types`）+ `package.json` + `cordis.patch.yml` + `README.md` 同步到 KCoder 仓库 `bundle/kcoder-file-review/`，提版触发物化重拷。运行时标识（typert 配对键、localStorage 存档键、locale NS、tab id `file-review`）与上游保持一致，用户状态不迁移。
