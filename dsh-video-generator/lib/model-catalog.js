...
const VIDEO = { imageToVideo: true, textToVideo: true, maxDurationSec: 10, qualityTier: 5 };
const IMAGE = { image: true, qualityTier: 5 };
...
export function resolveModel(model, override, builtin = BUILTIN_CATALOG) {
    ...
    if (hit)
        return { model, entry: hit, source: 'builtin' };
    return { model, entry: unknownEntry(), source: 'unknown' };
}
function matchBuiltin(id, rules) {
