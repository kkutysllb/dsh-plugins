...
export function locateFfmpeg(env = process.env) {
    if (env['VGEN_FFMPEG'])
        return env['VGEN_FFMPEG'];
    return 'ffmpeg';
}
...
export function drawtextMissing(stderr) {
...
export function buildRenderPlan(t, outPath, opts) {
    if (t.clips.length === 0)
        throw new Error('时间线没有 clip，无法渲染');
    ...
    t.subtitles.forEach((s, i) => {
        ...
        parts.push(`[${vLabel}]drawtext=${font}text='${escFilter(s.text)}':fontsize=48:fontcolor=white:borderw=3:bordercolor=black:x=(w-text_w)/2:y=h*0.82:enable='between(t,${from},${to})'[${out}]`);
        vLabel = out;
    });
    if (!useSubtitles) {
    ...
    if (totalUs > 0)
        args.push('-t', (totalUs / 1e6).toFixed(3));
...
function runOne(ffmpeg, args, timeoutMs = 300000) {
    return new Promise((resolve) => {
        execFile(ffmpeg, args, { timeout: timeoutMs, killSignal: 'SIGKILL', maxBuffer: 8 * 1024 * 1024 }, (err, _stdout, stderr) => {
            ...
            if (err)
                resolve({ ok: false, stderr: tail || String(err.message) });
            else
                resolve({ ok: true, stderr: tail });
        });
    });
}
export async function renderTimeline(t, outPath, opts = {}) {
    const ffmpeg = opts.ffmpeg ?? locateFfmpeg();
    if (!ffmpeg)
        return { ok: false, error: '未找到 ffmpeg（可设 VGEN_FFMPEG）' };
    ...
    try {
        ...
        for (const n of plan.normalize) {
            ...
            if (!r.ok)
                return { ok: false, error: `归一化失败: ${r.stderr}` };
        }
        const r = await runOne(ffmpeg, plan.composite.args, 600000);
        if (!r.ok) {
            if (drawtextMissing(r.stderr))
                return { ok: false, error: DRAWTEXT_HINT };
...
export async function probeDurationSec(file, ffmpeg = locateFfmpeg() ?? 'ffmpeg') {
    return new Promise((resolve) => {
