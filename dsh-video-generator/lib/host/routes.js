...
export function isLoopbackRequest(host, remote, trustedHosts = []) {
...
export function healthPayload(ctx) {
...
export function handleApi(ctx, name, args) {
    try {
        const value = dispatch(ctx, name, args);
        if (value instanceof Promise) {
            return value.then((v) => ({ ok: true, value: v }), (err) => {
...
function dispatch(ctx, name, args) {
    ...
    switch (name) {
        ...
        case 'runs.get': {
            ...
            if (!record)
                throw new VaultError('not-found', `run 不存在: ${rid}`);
            let entries = 0;
            let estCny = 0;
        ...
        case 'settings.update': {
            ...
            if (t !== undefined) {
                if (typeof t !== 'number' || !Number.isFinite(t))
                    throw new VaultError('bad-request', 'confirmThresholdCny 须为数字');
                ctx.vault.setBudget(t);
            }
            const gd = args['gateDefaults'];
            if (gd !== undefined) {
                if (typeof gd !== 'object' || gd === null || Array.isArray(gd))
                    throw new VaultError('bad-request', 'gateDefaults 须为对象 {段名: auto|ask|manual}');
                for (const [k, v] of Object.entries(gd)) {
                    if (!isStage(k))
                        throw new VaultError('bad-request', `gateDefaults 键须为合法段名: ${k}`);
                    if (v !== 'auto' && v !== 'ask' && v !== 'manual')
                        throw new VaultError('bad-request', `gateDefaults[${k}] 须为 auto|ask|manual`);
...
export function resolveMediaPath(runsRoot, urlPath) {
    const m = /^\/media\/([^/?#]+)\/(.+)$/.exec(urlPath);
    if (!m)
        return null;
    const runId = m[1];
    const rel = m[2];
    if (!MEDIA_RUNID_RE.test(runId))
        return null;
...
export function mediaContentType(filename) {
