...
function listDir(runDir, sub, prefix) {
...
function listTree(runDir, sub, prefix) {
    ...
    for (const e of entries) {
        ...
        if (statSync(full).isFile()) {
            ...
            for (const f of listDir(runDir, `${sub}/${e}`, `${prefix}/${e}`))
                out.push(f);
        }
    }
    return out;
}
function finalFile(runDir, name) {
    const full = join(runDir, name);
    if (!existsSync(full) || !statSync(full).isFile())
        return null;
    return { name, rel: name, size: statSync(full).size };
}
export function collectArtifacts(runs, runId) {
