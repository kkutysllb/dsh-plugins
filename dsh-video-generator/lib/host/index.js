...
import { probeChannel } from "../probe.js";
import { buildHandoffTools, handoffToolDefs } from "../tools/handoff.js";
import { buildGenerateTools, generateToolDefs } from "../tools/generate.js";
...
function registerHandoffTools(ctx, handoff) {
...
export function apply(ctx) {
    ...
    if (sectionAvailable) {
        disposers.push(ctx.systemPrompt.section({ name: `plugin:${PLUGIN_ID}`, order: SECTION_ORDER, text: vgenGuidance }));
    }
    ...
    ctx.effect(() => web.register({
        ...
        handler: async (req, res) => {
            ...
            if (!m) {
                json(res, 404, { ok: false, error: { code: 'not-found', message: '未知路径' } });
                return;
            }
...
function errorStatus(envelope) {
    const code = envelope.error?.code;
    if (code === 'not-found')
        return 404;
    if (code === 'conflict' || code === 'stale-revision' || code === 'proposal-stale' || code === 'project-exists' || code === 'proposal-limit')
        return 409;
    if (code === 'forbidden')
        return 403;
    if (code === 'too-large')
        return 413;
    if (code === 'bad-request' || code === 'workspace-unknown' || code === 'unknown-method' || code === 'bad-json')
        return 400;
    return 500;
}
...
async function readJsonBody(req, limitBytes = 1 << 20) {
    const chunks = [];
    let total = 0;
