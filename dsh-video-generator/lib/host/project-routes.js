...
function optionalRecord(v, field) {
    if (v === undefined)
        return undefined;
    if (typeof v !== 'object' || v === null || Array.isArray(v))
        throw new DramaError('bad-request', `字段 ${field} 须为对象`);
    return v;
}
/** 项目状态合成：待审核提案 > 存储推导状态（§2.3 徽章；已完成优先级最低）。 */
function composeStatus(base, pendingProposals) {
    if (pendingProposals > 0 && base !== 'done')
        return 'pending-review';
    return base;
}
function statusOf(detail) {
    const chapters = detail.chapters;
    if (detail.adaptations.length > 0)
        return 'adapting';
...
export function handleDramaApi(host, name, args) {
    try {
...
function dispatch(host, name, args) {
    switch (name) {
        ...
        case 'drama.project.create': {
            ...
            if (!p)
                throw new DramaError('bad-request', '字段 project 须为对象');
        ...
        case 'drama.asset.update': {
            const ws = host.requireProject(args['workspaceId'], args['projectId']);
            const { revision } = ws.projects.writeAsset(requireString(args['projectId'], 'projectId'), requireString(args['assetRef'], 'assetRef'), requireString(args['baseRevision'], 'baseRevision'), args['replacement']);
            return { revision };
        }
        case 'drama.candidate.save': {
            const ws = host.requireProject(args['workspaceId'], args['projectId']);
            const saved = ws.projects.saveCandidate(requireString(args['projectId'], 'projectId'), typeof args['chapter'] === 'number' ? args['chapter'] : Number.NaN, requireString(args['content'], 'content'));
            return saved;
        }
        case 'drama.candidate.list': {
            const ws = host.requireProject(args['workspaceId'], args['projectId']);
            const candidates = ws.projects.listCandidates(requireString(args['projectId'], 'projectId'), typeof args['chapter'] === 'number' ? args['chapter'] : Number.NaN);
            return { candidates };
        }
        ...
        case 'drama.proposal.apply': {
            const ws = host.requireProject(args['workspaceId'], args['projectId']);
            const { revision } = ws.proposals.apply(requireString(args['projectId'], 'projectId'), requireString(args['proposalId'], 'proposalId'), args['replacement']);
            return { revision };
        }
        case 'drama.proposal.reject': {
            const ws = host.requireProject(args['workspaceId'], args['projectId']);
            ws.proposals.reject(requireString(args['projectId'], 'projectId'), requireString(args['proposalId'], 'proposalId'), optionalString(args['note']));
            return {};
        }
        case 'drama.task.create': {
            ...
            if (!isTaskKind(kind))
                throw new DramaError('bad-request', `未知任务类型: ${kind}`);
        ...
        case 'drama.adaptation.create': {
            ...
            if (!/^\d{4}$/.test(chapterId))
                throw new DramaError('bad-request', `chapterId 须为 4 位章节号: ${chapterId}`);
            const params = optionalRecord(args['params'], 'params') ?? {};
            const detail = ws.projects.get(projectId);
            if (!detail)
                throw new DramaError('not-found', `项目不存在: ${projectId}`);
            const chapterText = chapterSourceText(ws, projectId, chapterId);
            const record = ws.projects.createAdaptation(projectId, { chapterId, params: params });
            const chapterNumber = Number(chapterId);
            const chapter = detail.chapters.find((c) => c.number === chapterNumber);
            const blueprint = chapter && chapter.blueprintRevision !== 'absent'
                ? readAssetOrNull(ws.projects, projectId, `chapters/${chapterId}/blueprint`)
                : null;
            const chars = asAssetField(detail.characters.data, 'characters')?.characters ?? [];
            const world = asAssetField(detail.worldbuilding.data, 'entries')?.entries ?? [];
            const { instruction, inputRefs } = assembleInstruction('adapt-chapter', {
                projectId,
                workspaceId: ws.id,
                premise: (detail.premise.data ?? null),
                architecture: (detail.architecture.data ?? null),
                outlineRow: null,
                blueprint,
                worldEntries: relevantWorldEntries(world),
                characters: characterSummaries(chars, blueprint?.characterIds ?? null, true),
                chapterNumber,
                adaptation: {
...
function asAssetField(data, field) {
...
function safeResolve(host, workspaceId) {
    try {
...
function readAssetOrNull(projects, projectId, assetRef) {
    const asset = projects.readAsset(projectId, assetRef);
    return asset.revision === 'absent' ? null : asset.data;
}
function chapterSourceText(ws, projectId, chapterId) {
    for (const part of ['final', 'draft']) {
        const asset = ws.projects.readAsset(projectId, `chapters/${chapterId}/${part}`);
        if (typeof asset.data === 'string' && asset.data.trim().length > 0)
            return asset.data;
    }
    throw new DramaError('bad-request', `第 ${Number(chapterId)} 章尚无定稿或草稿，不能创建改编任务`);
}
/** 按 kind 组装任务指令（§6.3）：Host 侧完成有限上下文裁剪；导出供单测直调。 */
export function assembleTaskInstruction(ws, projectId, kind, params, userRequest, detail) {
    const premise = (detail.premise.data ?? null);
    const architecture = (detail.architecture.data ?? null);
    const world = asAssetField(detail.worldbuilding.data, 'entries')?.entries ?? [];
    const chars = asAssetField(detail.characters.data, 'characters')?.characters ?? [];
    const outlineRows = asAssetField(detail.outline.data, 'rows')?.rows ?? [];
    ...
    if (cid && chapterNumber !== undefined) {
        ...
        if (kind === 'generate-chapter-draft' || kind === 'review-chapter' || kind === 'revise-chapter') {
            ctx.blueprint = readAssetOrNull(ws.projects, projectId, `chapters/${cid}/blueprint`) ?? null;
        }
        ...
        if (kind === 'generate-chapter-draft' && chapterNumber > 1) {
            ...
            if (prevCid) {
                ...
                const prevBlueprint = readAssetOrNull(ws.projects, projectId, `chapters/${prevCid}/blueprint`);
        ...
        if (kind === 'revise-chapter') {
            const review = readAssetOrNull(ws.projects, projectId, `chapters/${cid}/review`);
            const wanted = Array.isArray(params['problemIds'])
                ? params['problemIds'].filter((x) => typeof x === 'string')
                : null;
