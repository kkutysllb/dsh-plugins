 ...
 * Agent 须重读后再提案。
 */
import { DramaError, parseAssetRef, validateReplacement } from "../store/project.js";
/** 单资产读取上限（§6.1）：超过截断并报告。 */
export const DRAMA_READ_LIMIT_BYTES = 512 * 1024;
/** 读取有界化：内容超限截断，返回 truncated 标记与原始大小。 */
export function boundedContent(content) {
    const bytes = Buffer.byteLength(content, 'utf8');
    if (bytes <= DRAMA_READ_LIMIT_BYTES)
        return { content, truncated: false, originalBytes: bytes };
...
export function buildDramaTools(host) {
    ...
    return {
        ...
        propose: {
            execute: async (args) => wrap(() => {
                ...
                if (!summary.trim())
                    throw new DramaError('bad-request', '缺少 summary（提案摘要，供用户审核）');
                ...
                if (current !== baseRevision) {
                    throw new DramaError('stale-revision', `资产 ${assetRef} 当前 revision 与提交的 baseRevision 不一致（当前 ${current === 'absent' ? 'absent' : current.slice(0, 8)}），请先 drama_read 重读再提案`);
                }
...
const ASSET_REF_PARAM = {
    type: 'string',
    description: '资产引用（白名单）：premise | architecture | worldbuilding | outline | characters | chapters/<4位章节号>/<blueprint|draft|review|final>',
};
export function dramaToolDefs(tools) {
    return [
        {
            name: 'drama_read',
            description: '读取漫剧工坊项目的一个权威资产（内容 + revision）。单次最多一个资产；内容超过 512KiB 截断并报告。提案前必须先读取以获取最新 revision。',
            parameters: {
                type: 'object',
                properties: {
            ...
            timeoutMs: 10_000,
            execute: (args) => tools.read.execute((args ?? {})),
        },
        {
            name: 'drama_propose',
            description: '向漫剧工坊项目提交内容提案（pending），绝不直接写权威文件；用户在工坊页面审核（可编辑）后显式应用才生效。baseRevision 失配返回 stale-revision，须重读后再提案。一次只提案一个资产，replacement 必须是目标资产的完整替换内容（json 资产提交 JSON 文本，markdown 资产提交原文）。',
            parameters: {
                type: 'object',
                properties: {
            ...
            output: { schema: { type: 'object' }, render: jsonRender },
            timeoutMs: 10_000,
            execute: (args) => tools.propose.execute((args ?? {})),
        },
    ];
}
