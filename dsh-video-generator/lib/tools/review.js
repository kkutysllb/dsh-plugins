        ...
        review: {
            execute: async (args) => {
                try {
                    ...
                    if (!Number.isInteger(shot) || shot < 1 || shot > 200)
                        throw new HandoffError('bad-request', `shot 须为 1..200 整数: ${args?.shot}`);
                    ...
                    if (args?.score === undefined) {
                        if (!existsSync(clip))
                            throw new HandoffError('bad-request', `shot ${shot} 尚无成片片段：先完成 video 段（vgen_generate target=video）`);
                        if (!ffmpeg)
                            throw new Error('未找到 ffmpeg，无法抽帧（可设 VGEN_FFMPEG）');
                    ...
                    // ---- 重拍 ----
                    if (!existsSync(clip))
                        throw new HandoffError('bad-request', `shot ${shot} 尚无成片片段，无可重拍`);
                    const urlsEv = lastEventOf(record.events, 'shot-urls');
                    const urls = urlsEv?.detail?.urls ?? [];
                    ...
                    if (ffmpeg) {
                        try {
                            frames = await extract(clip, reviewDir, ffmpeg);
                        }
                        catch {
                            frames = [];
                        }
...
function pollDelayFromEnv(env) {
    const v = Number(env['VGEN_POLL_DELAY_MS']);
    return Number.isFinite(v) && v > 0 ? v : undefined;
}
export function reviewToolDefs(tools) {
    ...
    return [
        {
            name: 'vgen_review',
            description: '单镜质量评审闭环：不带 score 调用 → 返回成片 25/50/75% 三帧路径（用读图工具查看后评分）；带 score（1-5 整数）重调 → ≥3 记通过，≤2 自动追加负面词重拍（每镜至多 2 次；重拍花费走 confirm-required 语义）。评分与重拍次数记录进 run.json。',
            parameters: {
                type: 'object',
                properties: {
            ...
            timeoutMs: 600000,
            execute: (args) => tools.review.execute(args),
        },
    ];
}
