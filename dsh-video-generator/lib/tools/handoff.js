...
function adaptationRef(ctx, args) {
    const adaptationId = args['adaptationId'];
    if (adaptationId === undefined || adaptationId === null || adaptationId === '')
        return null;
    ...
    if (!ctx.drama)
        throw new HandoffError('bad-request', '当前宿主未启用漫剧工坊存储，不能携带 adaptationId');
    // not-found / workspace-unknown 语义集中在这里抛出
    const ws = ctx.drama.resolve(args['workspaceId']);
    ws.projects.requireProject(args['projectId']);
    return { ws, adaptationId, projectId: args['projectId'] };
}
function runDir(runs, runId) {
    return join(runs.rootDir, runId);
}
function persist(runs, runId, name, data) {
...
function requireRun(runs, runId) {
    const id = typeof runId === 'string' ? runId : '';
    if (!runs.get(id))
        throw new HandoffError('not-found', `run 不存在: ${id}`);
    return id;
}
function wrap(fn) {
    try {
...
function enrichShots(shots, characters, style) {
...
export function buildHandoffTools(ctx) {
    ...
    return {
        ...
        script: {
            execute: async (args) => wrap(() => {
                ...
                const ref = adaptationRef(ctx, (args ?? {}));
                if (ref)
                    ref.ws.projects.mirrorAdaptationArtifact(ref.projectId, ref.adaptationId, 'script', script);
        ...
        storyboard: {
            execute: async (args) => wrap(() => {
                ...
                const ref = adaptationRef(ctx, (args ?? {}));
                if (ref)
                    ref.ws.projects.mirrorAdaptationArtifact(ref.projectId, ref.adaptationId, 'storyboard', { shots: enriched });
...
export function handoffToolDefs(handoff) {
    return [
        {
            name: 'vgen_story',
            description: '提交结构化故事 JSON，开新 run 并落盘（LLM 三段交接第 1 步）。返回 runId 与阶段列表。漫剧改编任务（任务指令含 adaptationId）必须同时携带 workspaceId/projectId/adaptationId，工具会把产物镜像回项目改编任务并记录 run-link。',
            parameters: {
                type: 'object',
                properties: {
                    story: STORY_PARAM,
        ...
        },
        {
            ...
            output: { schema: { type: 'object' }, render: jsonRender },
            timeoutMs: 10_000,
            execute: (args) => handoff.script.execute(args),
        },
        {
            name: 'vgen_storyboard',
            description: '提交分镜数组（每镜 index/line/prompt/characterIds/sceneId/camera/durationSec 2-10/voiceHint），校验引用完整性并自动注入四层提示词，落盘 storyboard.json（LLM 三段交接第 3 步，之后接 vgen_generate 素材生成）。',
            parameters: {
                type: 'object',
                properties: {
                    runId: RUNID_PARAM,
                    shots: {
                        type: 'array',
                        description: '分镜数组：index 从 1 连续、line 镜头台词、prompt 手写画面描述、characterIds 引用 story 角色 id、sceneId 可选、camera 可选（≤200）、durationSec 2..10、voiceHint 可选',
                    },
            ...
            output: { schema: { type: 'object' }, render: jsonRender },
            timeoutMs: 10_000,
            execute: (args) => handoff.storyboard.execute(args),
        },
    ];
}
