...
export function buildChannelsTools(ctx) {
            ...
            execute: async (args) => {
                try {
                    ...
                    if (action === 'health') {
                        ...
                        if (!ch)
                            throw new HandoffError('not-found', id ? `通道不存在: ${id}` : '尚未配置任何通道（设置页「通道管理」或 channels.create）');
...
export function channelsToolDefs(tools) {
    ...
    return [
        {
            ...
            timeoutMs: 60000,
            execute: (args) => tools.channels.execute(args),
        },
    ];
}
