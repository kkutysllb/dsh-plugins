...
import { locateFfmpeg, probeDurationSec } from "../finalcut/render-ffmpeg.js";
import { HandoffError } from "../schema/handoff.js";
const PROVIDE_STAGES = ['master-asset', 'shot-assets', 'video', 'final-cut'];
const ASSET_NAME_RE = /^(char|scene)-[a-z0-9_-]+\.(png|jpe?g|webp)$/i;
const MIN_CLIP_SEC = 0.5;
function isProvideStage(s) {
    return typeof s === 'string' && PROVIDE_STAGES.includes(s);
}
function shotName(shot, ext) {
    return `shot-${String(shot).padStart(3, '0')}${ext}`;
}
export function buildProvideTools(ctx) {
            ...
            execute: async (args) => {
                try {
                    ...
                    if (!ctx.runs.get(runId))
                        throw new HandoffError('not-found', `run 不存在: ${runId}`);
                    const stage = args?.stage;
                    if (!isProvideStage(stage))
                        throw new HandoffError('bad-request', `stage 须为 ${PROVIDE_STAGES.join('|')}: ${String(args?.stage)}`);
                    const files = args?.files;
                    if (!Array.isArray(files) || files.length === 0 || files.length > 200)
                        throw new HandoffError('bad-request', 'files 须为 1..200 项数组');
                    // 通用存在性校验（先全验后拷贝，不半注入）
                    for (const f of files) {
                        if (typeof f?.path !== 'string' || !f.path)
                            throw new HandoffError('bad-request', 'files[].path 须为非空字符串');
                    ...
                    if (stage === 'master-asset') {
                        ...
                        const planned = files.map((f) => {
                            ...
                            if (!ASSET_NAME_RE.test(name))
                                throw new HandoffError('bad-request', `master-asset 文件名须为 char-<id>.png / scene-<id>.png 形态: ${f.name ?? '(缺)'}`);
                        ...
                        for (const f of files) {
                            ...
                            if (!Number.isInteger(shot) || shot < 1)
                                throw new HandoffError('bad-request', `shot-assets 每项须带 shot（≥1 整数）: ${JSON.stringify(f)}`);
                        ...
                        if (!existsSync(sbFile))
                            throw new HandoffError('bad-request', `run ${runId} 缺少 storyboard.json：请先完成分镜段`);
                        ...
                        const planned = files.map((f) => {
                            const shot = Number(f.shot);
                            if (!Number.isInteger(shot) || shot < 1)
                                throw new HandoffError('bad-request', `video 每项须带 shot（≥1 整数）: ${JSON.stringify(f)}`);
                        ...
                        for (const s of sb.shots) {
                            ...
                            if (providedIdx.has(s.index) || existsSync(p))
                                clipFiles.push(p);
                            else
                                missing.push(s.index);
                        }
                        if (missing.length)
                            throw new HandoffError('bad-request', `video 段缺少镜头: ${missing.join(', ')}（须覆盖 storyboard 全部 ${sb.shots.length} 镜）`);
                        ...
                        if (!ffmpeg)
                            throw new Error('未找到 ffmpeg，无法校验片段时长（可设 VGEN_FFMPEG）');
                        const checkDuration = async (file) => {
                            const dur = await probe(file, ffmpeg);
                            if (dur === null)
                                throw new HandoffError('bad-request', `无法读取片段时长: ${basename(file)}`);
                            if (dur < MIN_CLIP_SEC)
                                throw new HandoffError('bad-request', `片段 ${basename(file)} 时长 ${dur}s < ${MIN_CLIP_SEC}s`);
                        };
                        for (const p of planned)
                            await checkDuration(p.src);
                        // 此前已在位的 clip（非本次注入）同样过时长关，保持旧校验强度
                        for (const p of clipFiles) {
                            if (!planned.some((q) => q.dest === p))
                                await checkDuration(p);
                        }
                        mkdirSync(dest, { recursive: true, mode: 0o700 });
                        for (const p of planned) {
                            copyFileSync(p.src, p.dest);
                        ...
                        const mp4 = files[0];
                        if (extname(mp4.path).toLowerCase() !== '.mp4')
                            throw new HandoffError('bad-request', `final-cut 首文件须为 .mp4: ${basename(mp4.path)}`);
                        copyFileSync(mp4.path, join(runDir, 'final.mp4'));
                        ingested++;
                        const srt = files[1];
                        if (srt) {
                            if (extname(srt.path).toLowerCase() !== '.srt')
                                throw new HandoffError('bad-request', `final-cut 第二文件须为 .srt: ${basename(srt.path)}`);
...
export function provideToolDefs(tools) {
    ...
    return [
        {
            name: 'vgen_provide',
            description: 'manual gate 产物注入：把用户提供的本地文件校验后接管进管线。stage=master-asset（files[].name 须为 char-<id>.png/scene-<id>.png）；shot-assets/video（files[].shot 必填；video 须覆盖全部镜头且时长≥0.5s，需 ffmpeg）；final-cut（首文件 .mp4，可选第二文件 .srt，注入后 run 直接 done）。',
            parameters: {
                type: 'object',
                properties: {
            ...
            timeoutMs: 60000,
            execute: (args) => tools.provide.execute(args),
        },
    ];
}
