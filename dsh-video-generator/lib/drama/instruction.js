...
export function isTaskKind(v) {
    return typeof v === 'string' && TASK_KINDS.includes(v);
}
/** 各 kind 的提案目标资产（产出行的 drama_propose 落点）。 */
export function proposeTarget(kind, chapterNumber) {
...
export function relevantWorldEntries(entries) {
...
export function characterSummaries(characters, ids, withVisualPrompt) {
    const picked = ids ? characters.filter((c) => ids.includes(c.id)) : characters;
    const clip = (s) => (s.length > MAX_CHARACTER_SUMMARY_CHARS ? `${s.slice(0, MAX_CHARACTER_SUMMARY_CHARS)}…` : s);
...
export function prevChapterContext(prevFinal, prevBlueprint) {
...
function blueprintBrief(b) {
    if (!b)
        return '（本章蓝图未填写——请先基于大纲自行归纳本章目标，或提示用户补蓝图）';
    return [
        `本章目标：${b.goal || '（无）'}`,
        `冲突：${b.conflict || '（无）'}`,
        `场景：${b.scenes || '（无)'}`,
...
function charactersBrief(chars, withVisualPrompt) {
    if (chars.length === 0)
        return '（角色库为空）';
    return chars
        .map((c) => {
...
export function assembleInstruction(kind, ctx) {
...
function defaultRequirement(kind) {
    switch (kind) {
        case 'generate-architecture':
