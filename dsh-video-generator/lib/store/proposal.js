...
import { parseAssetRef, ProjectStore, validateReplacement, ABSENT_REVISION, DramaError } from "./project.js";
const PROPOSAL_ID_RE = /^prop-[0-9a-z][0-9a-z-]{0,63}$/;
const MAX_PENDING = 20;
const MAX_SUMMARY = 2000;
const MAX_NOTE = 2000;
function proposalKind(assetRef) {
...
function sanitizeProposal(v) {
    try {
        if (typeof v !== 'object' || v === null || Array.isArray(v))
            return null;
        const p = v;
        const s = (x, max) => (typeof x === 'string' ? x.slice(0, max) : '');
        const assetRef = s(p['assetRef'], 120);
        if (!parseAssetRef(assetRef))
            return null;
        const status = p['status'];
        if (status !== 'pending' && status !== 'applied' && status !== 'rejected' && status !== 'stale')
            return null;
...
export class ProposalStore {
    ...
    requireProposal(projectId, proposalId) {
        const p = this.read(projectId, proposalId);
        if (!p)
            throw new DramaError('not-found', `提案不存在: ${proposalId}`);
        return p;
    }
    ...
    create(projectId, input) {
        ...
        if (!parsed)
            throw new DramaError('bad-request', `未知资产: ${String(input.assetRef)}`);
        this.projects.requireProject(projectId);
    ...
    apply(projectId, proposalId, replacement) {
        ...
        if (!parsed)
            throw new DramaError('internal', `提案资产引用损坏: ${record.assetRef}`);
        const current = this.projects.revisionOf(parsed, projectId);
        if (current !== record.baseRevision) {
            record.status = 'stale';
            record.decidedAt = new Date().toISOString();
            this.write(record);
            throw new DramaError('proposal-stale', `提案已过期：资产 ${record.assetRef} 当前版本与提案基线不一致，请刷新后基于最新版本重新审阅或重新生成`);
        }
        ...
        if (replacement !== undefined)
            record.replacement = validateReplacement(parsed.label, parsed.kind, replacement);
        this.write(record);
        return { revision };
    }
    reject(projectId, proposalId, note) {
        const record = this.requireProposal(projectId, proposalId);
        if (record.status !== 'pending') {
        ...
        if (note !== undefined && note !== '')
            record.note = String(note).slice(0, MAX_NOTE);
        this.write(record);
    }
    /** 详情投影：fresh 标记供页面在应用前暴露「提案已过期」。 */
    summarize(projectId, record) {
        const parsed = parseAssetRef(record.assetRef);
