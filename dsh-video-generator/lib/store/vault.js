...
function sanitize(parsed) {
    ...
    if (typeof p.defaultChannelId === 'string' || p.defaultChannelId === null)
        d.defaultChannelId = p.defaultChannelId ?? null;
    ...
    deleteChannel(id) {
        this.mutate((d) => {
            ...
            if (d.channels.length === before)
                throw new VaultError('not-found', `通道不存在: ${id}`);
            if (d.defaultChannelId === id)
                d.defaultChannelId = d.channels[0]?.id ?? null;
        });
    }
    setDefaultChannel(id) {
        this.mutate((d) => {
            if (id !== null && !d.channels.some((c) => c.id === id))
                throw new VaultError('not-found', `通道不存在: ${id}`);
            d.defaultChannelId = id;
        });
    }
...
function validateModels(models) {
    if (!Array.isArray(models) || models.length > 100)
        throw new VaultError('bad-request', 'models 须为数组且 ≤100');
    const seen = new Set();
    return models.map((m) => {
        const model = String(m?.model ?? '');
        if (model.length < 1 || model.length > 200)
            throw new VaultError('bad-request', `非法模型名: ${m?.model}`);
        if (seen.has(model))
            throw new VaultError('bad-request', `重复模型名: ${model}`);
        seen.add(model);
        if (!['image', 'video', 'tts'].includes(m?.kind))
            throw new VaultError('bad-request', `非法模型 kind: ${m?.kind}`);
        const out = { model, kind: m.kind };
        if (m.endpointProfile !== undefined)
            out.endpointProfile = String(m.endpointProfile).slice(0, 120);
        if (m.pricingCny !== undefined) {
            const p = Number(m.pricingCny);
            if (!Number.isFinite(p) || p < 0)
                throw new VaultError('bad-request', `非法 pricingCny: ${m.pricingCny}`);
            out.pricingCny = p;
        }
        if (m.qualityTier !== undefined) {
            const q = Number(m.qualityTier);
            if (!Number.isInteger(q) || q < 0 || q > 10)
                throw new VaultError('bad-request', `非法 qualityTier: ${m.qualityTier}`);
            out.qualityTier = q;
        }
        return out;
    });
