...
function boundedStr(v, path, max) {
    const s = str(v, path);
    if (s.length > max)
        fail(`${path} 长度超限：实际 ${s.length}，上限 ${max}`);
    return s;
}
/** 角色引用 id：^[a-z0-9_-]+$ 格式 + ≤48。 */
function idStr(v, path) {
    const s = boundedStr(v, path, MAX.id);
    if (!ID_RE.test(s))
        fail(`${path} 须匹配 ^[a-z0-9_-]+$: ${s}`);
    return s;
}
function strArray(v, path) {
    if (!Array.isArray(v))
        fail(`${path} 须为数组`);
    return v.map((x, i) => str(x, `${path}[${i}]`));
}
/** 角色表解析（story/script/storyboard 复用）：非空数组、逐条 {id,name,appearance}、id 格式 + 唯一。 */
function parseCharacters(raw, path, opts) {
    if (!Array.isArray(raw))
        fail(`${path} 须为数组`);
    if (opts.requireNonEmpty && raw.length === 0)
        fail(`${path} 须为非空数组`);
    if (raw.length > MAX.characters)
        fail(`${path} 数量超限：实际 ${raw.length}，上限 ${MAX.characters}`);
    const characters = raw.map((c, i) => {
        ...
        if (cr['voiceHint'] !== undefined)
            character.voiceHint = boundedStr(cr['voiceHint'], `${path}[${i}].voiceHint`, MAX.voiceHint);
        return character;
    });
    const ids = new Set(characters.map((c) => c.id));
    if (ids.size !== characters.length)
        fail(`${path} 存在重复 id`);
    return characters;
}
/** 场景表解析（script/storyboard 复用）：逐条校验 + id 格式 + 唯一；storyboard 里允许整表缺省（不提供即 []）。 */
function parseScenes(raw, path, opts) {
    if (raw === undefined && !opts.requireNonEmpty)
        return [];
    if (!Array.isArray(raw))
        fail(`${path} 须为数组`);
    if (opts.requireNonEmpty && raw.length === 0)
        fail(`${path} 须为非空数组`);
    if (raw.length > MAX.scenes)
        fail(`${path} 数量超限：实际 ${raw.length}，上限 ${MAX.scenes}`);
...
export function validateStory(v, pathPrefix = 'story') {
    ...
    if (chapters.length > MAX.chapters)
        fail(`${pathPrefix}.chapters 数量超限：实际 ${chapters.length}，上限 ${MAX.chapters}`);
    return {
...
export function validateScript(v) {
    ...
    for (let i = 0; i < scenes.length; i++) {
        ...
        for (const cid of s.characters) {
            if (!charIds.has(cid))
                fail(`script.scenes[${i}].characters 引用不存在的 characterId: ${cid}`);
        }
    }
    const dialogRaw = r['dialog'];
    if (!Array.isArray(dialogRaw))
        fail('script.dialog 须为数组');
    if (dialogRaw.length > MAX.dialog)
        fail(`script.dialog 数量超限：实际 ${dialogRaw.length}，上限 ${MAX.dialog}`);
    const sceneIds = new Set(scenes.map((s) => s.id));
    const dialog = dialogRaw.map((d, i) => {
        ...
        if (!sceneIds.has(sceneId))
            fail(`script.dialog[${i}].sceneId 引用不存在的场景: ${sceneId}`);
...
export function validateStoryboard(v) {
    ...
    if (!Array.isArray(shotsRaw) || shotsRaw.length === 0)
        fail('storyboard.shots 须为非空数组');
    if (shotsRaw.length > MAX.shots)
        fail(`storyboard.shots 数量超限：实际 ${shotsRaw.length}，上限 ${MAX.shots}`);
    ...
    const shots = shotsRaw.map((s, i) => {
        ...
        if (!Number.isInteger(index) || index !== i + 1)
            fail(`storyboard.shots[${i}].index 须为连续序号（从 1 开始），期望 ${i + 1}`);
        ...
        if (sr['camera'] !== undefined)
            shot.camera = boundedStr(sr['camera'], `storyboard.shots[${i}].camera`, MAX.camera);
        if (sr['voiceHint'] !== undefined)
            shot.voiceHint = boundedStr(sr['voiceHint'], `storyboard.shots[${i}].voiceHint`, MAX.voiceHint);
        return shot;
    });
