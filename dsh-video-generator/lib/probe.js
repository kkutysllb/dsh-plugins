...
export async function probeChannel(target, fetchImpl = fetch, timeoutMs = 15000) {
    ...
    try {
        ...
        const models = Array.from(new Set(raw.map((m) => (typeof m === 'string' ? m : m?.id ?? ''))))
            .filter((s) => s.length > 0)
            .sort();
        if (!models.length)
            return { ok: false, baseUrl: base, models: [], status: res.status, error: 'no-models' };
