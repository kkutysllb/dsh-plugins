...
```bash
cd <本技能包根>/scripts && python3 financial_cli.py 600519.SH
...
脚本内部自动安装 common 依赖（`pip install -e ../common`），无需手动操作；`TUSHARE_TOKEN` 从凭据文件注入运行环境（运行前 source 凭据文件，见「凭据配置」），脚本直接读取。数据缺失时脚本会如实标注「缺失」，**禁止编造数据**；如需进一步解读（勾稽关系、评分卡、红旗深入分析），基于脚本输出做分析即可。
