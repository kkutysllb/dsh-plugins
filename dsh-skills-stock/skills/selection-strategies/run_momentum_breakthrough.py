...
策略特点：
  - 基于价格动量和技术突破信号
  - RSI 50~70、量比 >= 1.5、MACD金叉
  - 重点关注量价配合的突破股票，适合短中线趋势跟随

用法：
  python scripts/run_momentum_breakthrough.py [选项]
...
def print_results(result: dict):
    ...
    headers = ["排名", "代码", "名称", "综合分", "RSI", "量比", "涨跌幅%", "20日动量%"]
    ...
    for idx, s in enumerate(stocks, 1):
        ...
        row = [
            ...
            _f(s.get('pct_chg')),
            _f(s.get('momentum_20', s.get('momentum'))),
        ]
...
async def main():
    parser = argparse.ArgumentParser(description='动量突破策略选股')
