...
策略特点：
  - 多维度识别超跌反弹机会
  - RSI < 50、量比 >= 1.0，筛选极度超卖后出现量能的个股
  - 适合短线波段操作

用法：
  python scripts/run_oversold_rebound.py [选项]

选项：
  --limit          N     返回股票数量（默认 20）
  --market-cap     TYPE  市值范围：large / mid / small / all（默认 all）
  --stock-pool     TYPE  股票池：all / main / gem / star（默认 all）
  --rsi-max        N     RSI上限（默认 50）
  --rsi-min        N     RSI下限（默认 10）
  --volume-ratio   N     最小量比（默认 1.0）
  --pe-max         N     PE上限（默认 100）
...
def print_header(strategy_name: str, params: dict):
    ...
    print(f"  参数：市值范围={params.get('market_cap')}  RSI范围={params.get('rsi_min')}~{params.get('rsi_max')}  量比>={params.get('volume_ratio_min')}")
    print("=" * 65)
...
def print_results(result: dict):
    ...
    col_w = [6, 10, 8, 8, 7, 7, 8, 8]
    headers = ["排名", "代码", "名称", "综合分", "RSI", "量比", "涨跌幅%", "超跌幅%"]
    ...
    for idx, s in enumerate(stocks, 1):
        ...
        row = [
            ...
            _f(s.get('pct_chg'), '.2f'),
            _f(s.get('oversold_pct', s.get('decline_pct'))),
        ]
...
async def main():
    parser = argparse.ArgumentParser(description='超跌反弹策略选股')
    ...
    parser.add_argument('--rsi-max',      type=float, default=50.0,  dest='rsi_max',   help='RSI上限')
    parser.add_argument('--rsi-min',      type=float, default=10.0,  dest='rsi_min',   help='RSI下限')
    parser.add_argument('--volume-ratio', type=float, default=1.0,   dest='volume_ratio_min', help='最小量比')
    parser.add_argument('--pe-max',       type=float, default=100.0, dest='pe_max',    help='PE上限')
    ...
    params = {
        'market_cap': args.market_cap, 'rsi_min': args.rsi_min,
        'rsi_max': args.rsi_max, 'volume_ratio_min': args.volume_ratio_min
    }
    print_header("超跌反弹策略", params)
