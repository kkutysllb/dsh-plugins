...
- **🌿 Git 面板**：真 diff + VSCode 式 diff tab、历史、右键暂存 / 提交 / 还原；工作区容器下自动发现子仓库并显示**仓库选择器**，支持 linked worktree 变更发现。面板顶部「变更 / 分支 / GitHub」三视图：变更视图带**上游距离**（↑ahead ↓behind / 已同步 / 待推送 / 无上游）、**推送 / 提交并推送**（无上游自动 `push -u`）与**逐文件 `+N −M`** 行数（未跟踪文件按正文计行）；分支视图支持搜索、新建并检出、检出远程短名（建跟踪分支）与删除（安全删被拒时升级为二次确认的强制删）；GitHub 视图走 `gh` CLI 软依赖（缺席 / 未登录可读降级 + 重新探测）——开放 PR / Issue 列表与计数、当前分支命中标记、一键创建 PR（缺上游自动推送）、Squash/Merge/Rebase 合并（行内二次确认）与新建 Issue，均从系统浏览器打开
- **📋 任务计划**：把工作区里 agent 写的计划文档扫成一个独立 Tab——约定位置 `plans/` · `docs/plans/` · `.plans/`（一层 `*.md`）与根 `plan.md` / `PLAN.md` / `docs/plan.md`，按 `dev:ino` 身份去重（大小写不敏感盘上 `plan.md` 与 `PLAN.md` 是同一文件）后按修改时间倒序；行标题取文档首个 `#`~`###` 标题（只读 512B 头部，无标题回退文件名），行副标题显示相对路径与相对时间；点击在侧边栏自己的编辑器里打开，行尾按钮可交给**系统默认应用**（宿主侧限定工作区内 + 文本扩展名白名单）；可见时 4s 轮询，支持搜索与手动刷新
