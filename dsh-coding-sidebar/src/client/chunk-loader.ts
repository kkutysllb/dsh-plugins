 ...
 * libraries (CodeMirror, xterm — the editor/terminal stacks, several MB, and
 * the Office stack — Univer / docx-preview / pptx-renderer, ~22MB) live in
 * separate build-time bundles (`lib/client-<name>.js`) fetched only on first
 * use of the feature that needs them, so startup downloads/parses only the
 * ~1MB core bundle.
 *
 ...
 */
export type ChunkName = 'terminal' | 'editor' | 'locale' | 'trajectory' | 'mermaid' | 'office'
