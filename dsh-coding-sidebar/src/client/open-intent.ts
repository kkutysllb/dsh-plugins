 ...
 * @returns 内容型为 `true`。
 */
export function isContentOpen(seed: OpenIntentSeed): boolean {
  return seed.path !== undefined || seed.url !== undefined || seed.meta !== undefined
}
 ...
 */
export function needsPanelExpansion(seed: OpenIntentSeed, context: OpenIntentContext): boolean {
  return isContentOpen(seed)
    && !context.targetsInactiveSession
    && context.hasWindow
    && !context.panelOpen
}
